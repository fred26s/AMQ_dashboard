...
import { formatTimeDifference , calculateFee} from '../utils/lib'
...
const tradeTimesTips = computed(
  () => `won: ${toValue(tradeTimesWon)}; lost: ${toValue(tradeTimesLost)}`
)
const tradePctWon = computed(
  () => `${Number((toValue(tradeTimesWon) / toValue(tradeTimes)) * 100).toFixed(2)}%`
)
...
const chartXdata = computed(() =>
  toValue(tradesList).map((e) => new Date(e.sell.timestamp).toLocaleString())
)
const chartYdata1 = computed(() =>
  toValue(tradesList).map((e) => {
    return {
...
// 手续费
const totalFee = computed(() => toValue(tradesList).map(e => {
  return calculateFee(formatTimeDifference(e.sell.timestamp, e.buy.timestamp).hours, e.buy.cost).total
}).reduce((a, b) => Number(a) + Number(b), 0))
// 胜率统计
const longWin = computed(
  () => toValue(tradesList).filter((e) => e.extra.direction === 'long' && e.profit.amt > 0).length
)
const longTimes = computed(
  () => toValue(tradesList).filter((e) => e.extra.direction === 'long').length
)

const shortWin = computed(
  () => toValue(tradesList).filter((e) => e.extra.direction === 'short' && e.profit.amt > 0).length
)
const shortTimes = computed(
  () => toValue(tradesList).filter((e) => e.extra.direction === 'short').length
)
...
<template>
  <div class="container mx-auto">
    <div class="flex flex-row flex-wrap mt-3 md:justify-between md:flex-nowrap">
      ...
      <statsCard label="手续费" :value="totalFee"></statsCard>
      <statsCard label="胜率" :value="tradePctWon" tips=""></statsCard>
      <statsCard label="交易次数" :value="tradeTimes" :tips="tradeTimesTips"></statsCard>
      <statsCard
        label="做多胜率"
        :value="`${Math.floor((longWin / longTimes) * 100)}%`"
        :tips="`${longWin}/${longTimes}`"
      ></statsCard>
      <statsCard
        label="做空胜率"
        :value="`${Math.floor((shortWin / shortTimes) * 100)}%`"
        :tips="`${shortWin}/${shortTimes}`"
      ></statsCard>
    </div>
    <div class="flex mt-3">
